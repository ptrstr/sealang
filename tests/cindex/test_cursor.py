...
from clang.cindex import (
    AvailabilityKind, CursorKind, TemplateArgumentKind, 
    TranslationUnit, TypeKind, BinaryOperator, UnaryOperator
)
            ...
            for cursor in tu.cursor.walk_preorder():
                if cursor.unary_operator != UnaryOperator.UNKNOWN:
                    if cursor.unary_operator == operator:
                        self.assertEqual(cursor.operator, spelling)
                        found = True
            if not found:
                self.fail(f"Operator {operator} ({spelling}) not found in test data")
        ...
        for literal, spelling in literals.items():
            ...
            for cursor in tu.cursor.walk_preorder():
                if cursor.kind == literal:
                    self.assertEqual(cursor.literal, spelling)
                    found = True
            if not found:
                self.fail(f"{literal} '{spelling}' not found in test data")
